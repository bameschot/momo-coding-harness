...
- `grep_extract` — pull values out of structured text (a capture group), not whole lines
...
Example: `<tool_call>{"name": "grep_extract", "arguments": {"pattern": "(\\d{4}-\\d{2}-\\d{2})", "path": "log.txt", "group": 1}}</tool_call>`
